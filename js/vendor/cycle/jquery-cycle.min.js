...
}g("first timeout: "+x);this.cycleTimeout=setTimeout(function(){n(w,v,0,!y.backwards);},x);}});};function k(s,v,u){var w=c(s).data("cycle.opts");if(!w){return;
}var t=!!s.cyclePause;if(t&&w.paused){w.paused(s,w,v,u);}else{if(!t&&w.resumed){w.resumed(s,w,v,u);}}}function l(s,v,t){if(s.cycleStop===d){s.cycleStop=0;
}if(v===d||v===null){v={};}if(v.constructor==String){switch(v){case"destroy":case"stop":var x=c(s).data("cycle.opts");if(!x){return false;}s.cycleStop++;
if(s.cycleTimeout){clearTimeout(s.cycleTimeout);}s.cycleTimeout=0;if(x.elements){c(x.elements).stop();}c(s).removeData("cycle.opts");if(v=="destroy"){r(s,x);
}return false;case"toggle":s.cyclePause=(s.cyclePause===1)?0:1;w(s.cyclePause,t,s);k(s);return false;case"pause":s.cyclePause=1;k(s);return false;case"resume":s.cyclePause=0;
w(false,t,s);k(s);return false;case"prev":case"next":x=c(s).data("cycle.opts");if(!x){f('options not found, "prev/next" ignored');return false;}c.fn.cycle[v](x);
return false;default:v={fx:v};}return v;}else{if(v.constructor==Number){var u=v;v=c(s).data("cycle.opts");if(!v){f("options not found, can not advance slide");
return false;}if(u<0||u>=v.elements.length){f("invalid slide index: "+u);return false;}v.nextSlide=u;if(s.cycleTimeout){clearTimeout(s.cycleTimeout);s.cycleTimeout=0;
}if(typeof t=="string"){v.oneTimeFx=t;}n(v.elements,v,1,u>=v.currSlide);return false;}}return v;function w(z,A,y){if(!z&&A===true){var B=c(y).data("cycle.opts");
if(!B){f("options not found, can not resume");return false;}if(y.cycleTimeout){clearTimeout(y.cycleTimeout);y.cycleTimeout=0;}n(B.elements,B,1,!B.backwards);
}}}function e(s,t){if(!c.support.opacity&&t.cleartype&&s.style.filter){try{s.style.removeAttribute("filter");}catch(u){}}}function r(s,t){if(t.next){c(t.next).unbind(t.prevNextEvent);
}if(t.prev){c(t.prev).unbind(t.prevNextEvent);}if(t.pager||t.pagerAnchorBuilder){c.each(t.pagerAnchors||[],function(){this.unbind().remove();});}t.pagerAnchors=null;
c(s).unbind("mouseenter.cycle mouseleave.cycle");if(t.destroy){t.destroy(t);}}function q(C,Q,z,y,J){var s;var H=c.extend({},c.fn.cycle.defaults,y||{},c.metadata?C.metadata():c.meta?C.data():{});
